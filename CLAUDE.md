...
- **Next waves:** W1 remaining = 0.3 QDS import (Backend, unblocked by 0.2 schemas) · docs scaffold (Documentation). Still open from 0.1: owners for `content/`, `tools/pack-builder`, `packages/tajweed-rules`, `packages/plugin-sdk`; §21/§28 say `OWNER.md` but task 0.1 specified `README.md` — owner lines live in the READMEs pending an ADR. DevOps should fold `schemas-codegen.yml` into the main `ci.yml` path-filtered pipeline.
